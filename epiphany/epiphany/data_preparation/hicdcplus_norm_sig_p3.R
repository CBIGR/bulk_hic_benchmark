...
outdir <- "/scratch/gent/vo/000/gvo00027/projects/CBIGR/24NHT_master_thesis/code/epiphany/ground_truth/features/" # path to save the constructed features
outpth <- "/scratch/gent/vo/000/gvo00027/projects/CBIGR/24NHT_master_thesis/code/epiphany/ground_truth/intermediate/" # path to save the results
chromosomes <- paste("chr", seq(1, 22, 1), sep = "") # which chromosomes to normalize # nolint
merged_location <- "/scratch/gent/vo/000/gvo00027/projects/CBIGR/24NHT_master_thesis/code/epiphany/ground_truth/" # path of aligned hic-pro files # nolint
sample_names <- c("GM12878") # Hi-C file names
hg_assembly <- "hg38"
species <- "Hsapiens"
...
for (sample_name in sample_names){
    hicfile_path <- paste0(merged_location, sample_name, "_gt" , ".hic", sep = "")
    gi_list <- generate_bintolen_gi_list(bintolen_path = paste0(outdir, "/", hg_assembly, "_", as.integer(resolution / 1000),"kb_GATC_GANTC_bintolen.txt.gz", sep = ""))
    ...
    gi_list_write(gi_list, fname = paste0(outpth, sample_name, "_", resolution, "_result.txt.gz", sep = ""), score = "normcounts")
    }