...
parser.add_argument("--hic_resolution", help="Resolution of the Hi-C map being normalized", type=int)
            ...
            for line in islice(fin, skip_row, None):
                ...
                if line_split[0] == line_split[3] == chrom:
                    ...
                    if dtype == "z-value":
                        ...
                        value = float(float(line_split[10]) / (float(line_split[10]) + 1e-10))
                    contact_matrix[int(int(idx1)/res)][int(int(idx2)/res)] = value
