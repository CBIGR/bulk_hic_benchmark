...
for chrom, chrom_size in chrom_sizes.items():
    ...
    files = sorted([f for f in os.listdir(data_dir) if f.startswith(chrom + "_") and f.endswith(".npy")])

    for fname in tqdm(files, desc="Merging submatrices"):
        # Extract start bin from filename
        try:
            # Expected format: chr1_00000000.npy
            start_bp = int(fname.replace(f"{chrom}_", "").replace(".npy", ""))
        except ValueError:
        ...
        # Load submatrix
        submatrix = np.load(os.path.join(data_dir, fname))
...
min_count_general = merge_bedpe['count'].min()
if min_count_general < 0 :
    merge_bedpe['count'] = merge_bedpe['count']+abs(min_count_general) 
merge_bedpe.to_csv(f"{output_dir}/{dataset}_{resolution}_count.bedpe", sep="\t", header=False, index=False)

merge_bedpe_norm = pd.concat(bedpes_norm, ignore_index=True)
min_norm_general = merge_bedpe_norm['count'].min()
if min_norm_general < 0 :
    merge_bedpe_norm['count'] = merge_bedpe_norm['count']+abs(min_norm_general) 
merge_bedpe_norm.to_csv(f"{output_dir}/{dataset}_{resolution}_norm.bedpe", sep="\t", header=False, index=False)