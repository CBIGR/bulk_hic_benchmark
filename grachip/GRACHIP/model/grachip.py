...
class network(torch.nn.Module):
    def __init__(self,indexsize,gcnInputsize,gcnHiddensize,gcnOutputsize,batchSize):
        ...
        if torch.cuda.is_available():
            self.encoder.load_state_dict(torch.load("/scratch/gent/vo/000/gvo00027/projects/CBIGR/24NHT_master_thesis/code/grachip/grachip-dataset/models/encoder.pt"))
        else:
            self.encoder.load_state_dict(torch.load("/scratch/gent/vo/000/gvo00027/projects/CBIGR/24NHT_master_thesis/code/grachip/grachip-dataset/models/encoder.pt",map_location=torch.device('cpu')))
        self.encoder.eval()
